...
@app.route('/hzz/update', methods=['POST', ])
def hzz_project_update():
    projectdb = app.config['projectdb']
...
@app.route('/hzz/run', methods=['POST', ])
def hzz_runtask():
    rpc = app.config['scheduler_rpc']
    if rpc is None:
        return json.dumps({})
...
@app.route('/hzz/robots.txt')
def hzz_robots():
    return """User-agent: *
Disallow: /
Allow: /$
Allow: /hzz/debug
Disallow: /hzz/debug/*?taskid=*
""", 200, {'Content-Type': 'text/plain'}

@app.route('/hzz/delete')
def hzz_delete():
