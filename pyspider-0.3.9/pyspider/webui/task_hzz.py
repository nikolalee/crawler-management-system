...
def hzz_task(taskid):
    ...
    return render_template("task_hzz.html", task=task, json=json, result=result,
                           status_to_string=app.config['taskdb'].status_to_string)


@app.route('/hzz/task/<taskid>.json')
def hzz_task_in_json(taskid):
    if ':' not in taskid:
...
@app.route('/hzz/active_tasks')
def hzz_active_tasks():
    rpc = app.config['scheduler_rpc']
    taskdb = app.config['taskdb']
    project = request.args.get('project', "")
    limit = int(request.args.get('limit', 100))
