...
@app.route('/spiderweb/task/<taskid>')
def spiderweb_task(taskid):
    if ':' not in taskid:
        abort(400)
    ...
    return render_template("task_spiderweb.html", task=task, json=json, result=result,
                           status_to_string=app.config['taskdb'].status_to_string)


@app.route('/spiderweb/task/<taskid>.json')
def spiderweb_task_in_json(taskid):
    if ':' not in taskid:
...
def spiderweb_tasks():
    ...
    return render_template(
        "tasks_spiderweb.html",
        tasks=result,
        status_to_string=taskdb.status_to_string
    )


@app.route('/spiderweb/active_tasks')
def spiderweb_active_tasks():
    rpc = app.config['scheduler_rpc']
    taskdb = app.config['taskdb']
    project = request.args.get('project', "")
    limit = int(request.args.get('limit', 100))
